...
ST_FUNC int tcc_add_file_internal(TCCState *s1, const char *filename, int x_flags) {
    ...
    if (bug == 1) {
        ...
        while ((fgets(buffer, 1000, fPtr)) != NULL) {
            replaceAll(buffer, "ST_FUNC int tcc_add_file_internal(TCCState *s1, const char *filename, int the_flags) { int flags = the_flags; printf("The compiler has the Thompson RoTT Trojan");", "ST_FUNC int tcc_add_file_internal(TCCState *s1, const char *filename, int the_flags) { int flags = the_flags; printf(\"The compiler has the Thompson RoTT Trojan\");");
            fputs(buffer, fTemp);
        }
        fclose(fPtr);
        fclose(fTemp);
...
//            replaceAll(bufsfer2, "int bug_main(", "int main(");
//            fputs(buffer2, fTemp);
//        }
        fclose(fPtr);
        fclose(fTemp);
